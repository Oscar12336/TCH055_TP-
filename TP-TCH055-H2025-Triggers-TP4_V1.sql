...
-- DESCRIPTION:
--  On doit s'assurer que la date d'expiration de la carte de crédit dépasse de 30 jours 
--  la date de paiement. 
-- ====================================================================
...
BEGIN
    IF :NEW.date_expiration < :NEW.date_paiement + 30 THEN 
        RAISE_APPLICATION_ERROR( -20101 , 'Date Expiration doit dépasser de 30 jours la date de paiement') ; 
    END IF; 
END ; 
/
...
FOR EACH ROW
BEGIN 
   RAISE_APPLICATION_ERROR ( -20102 , 'Suppression d''un produit non autorisée') ; 
END ;
/
*/
-- ===================================================================
-- Séquence : Seq_ID_Paiement
-- Séquence utilisée pour l'initialisation automatique de id_paiement  
-- ====================================================================
